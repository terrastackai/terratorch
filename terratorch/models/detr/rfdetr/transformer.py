...
def gen_sineembed_for_position(pos_tensor, dim=128):
    ...
    if pos_tensor.size(-1) == 2:  # noqa: PLR2004
        pos = torch.cat((pos_y, pos_x), dim=2)
    elif pos_tensor.size(-1) == 4:  # noqa: PLR2004
...
def gen_encoder_output_proposals(
    ...
    memory_padding_mask,
    spatial_shapes,
    unsigmoid=True,  # noqa: FBT002
):
    r"""Generate encoder output proposals.
    ...
    """
    N_ = memory.shape[0]  # noqa: N806
    proposals = []
    _cur = 0
    for lvl, (H_, W_) in enumerate(spatial_shapes):  # noqa: N806
        if memory_padding_mask is not None:
            mask_flatten_ = memory_padding_mask[:, _cur : (_cur + H_ * W_)].view(N_, H_, W_, 1)
            valid_H = torch.sum(~mask_flatten_[:, :, 0, 0], 1)  # noqa: N806
            valid_W = torch.sum(~mask_flatten_[:, 0, :, 0], 1)  # noqa: N806
        else:
            valid_H = torch.tensor([H_ for _ in range(N_)], device=memory.device)  # noqa: N806
            valid_W = torch.tensor([W_ for _ in range(N_)], device=memory.device)  # noqa: N806
    ...
    _proposal_lower = 0.01
    _proposal_upper = 0.99
    output_proposals_valid = ((output_proposals > _proposal_lower) & (output_proposals < _proposal_upper)).all(
        -1, keepdim=True
    )
...
class Transformer(nn.Module):
    def __init__(
        ...
        dropout=0.0,
        activation="relu",
        normalize_before=False,  # noqa: FBT002
        ...
        if decoder_norm_type not in ("LN", "Identity"):
            msg = f"decoder_norm_type must be 'LN' or 'Identity', got '{decoder_norm_type}'"
            raise ValueError(msg)
        norm = {
            "LN": nn.LayerNorm,
            "Identity": lambda _channels: nn.Identity(),
        }
    ...
    def get_valid_ratio(self, mask):
        _, H, W = mask.shape  # noqa: N806
        valid_H = torch.sum(~mask[:, :, 0], 1)  # noqa: N806
        valid_W = torch.sum(~mask[:, 0, :], 1)  # noqa: N806
    ...
    def forward(self, srcs, masks, pos_embeds, refpoint_embed, query_feat):
        ...
        for _lvl, (src, pos_embed) in enumerate(zip(srcs, pos_embeds, strict=False)):
            bs, _c, h, w = src.shape
            spatial_shape = (h, w)
            spatial_shapes.append(spatial_shape)

            src = src.flatten(2).transpose(1, 2)  # bs, hw, c  # noqa: PLW2901
            pos_embed = pos_embed.flatten(2).transpose(1, 2)  # bs, hw, c  # noqa: PLW2901
            lvl_pos_embed_flatten.append(pos_embed)
            src_flatten.append(src)
            if masks is not None:
                mask = masks[_lvl].flatten(1)  # bs, hw
        ...
        if self.dec_layers > 0:
            ...
            if not self.two_stage:
                msg = "if not using decoder, two_stage must be True"
                raise ValueError(msg)
            hs = None
            references = None
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        num_layers,
        norm=None,
        return_intermediate=False,  # noqa: FBT002
        d_model=256,
        lite_refpoint_refine=False,  # noqa: FBT002
        bbox_reparam=False,  # noqa: FBT002
    ):
    ...
    def forward(
        ...
        memory_mask: Tensor | None = None,  # noqa: ARG002
        ...
        if self.lite_refpoint_refine:
            if self.bbox_reparam:
                _obj_center, refpoints_input, query_pos, query_sine_embed = get_reference(refpoints_unsigmoid)
            else:
                _obj_center, refpoints_input, query_pos, query_sine_embed = get_reference(refpoints_unsigmoid.sigmoid())
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if not self.lite_refpoint_refine:
                if self.bbox_reparam:
                    ...
                    _obj_center, refpoints_input, query_pos, query_sine_embed = get_reference(
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(
        ...
        tgt_key_padding_mask: Tensor | None = None,
        memory_key_padding_mask: Tensor | None = None,
        pos: Tensor | None = None,  # noqa: ARG002
        query_pos: Tensor | None = None,
        query_sine_embed=None,  # noqa: ARG002
        is_first=False,  # noqa: ARG002, FBT002
...
def _get_clones(module, n):
    return nn.ModuleList([copy.deepcopy(module) for _i in range(n)])
