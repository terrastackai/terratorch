...
from terratorch.models.detr.rfdetr.ops.functions import ms_deform_attn_core_pytorch
...
class MSDeformAttn(nn.Module):
    ...
    def forward(
        ...
        input_level_start_index,  # noqa: ARG002
        ...
        Args:
            ...
            input_level_start_index: (n_levels, ), [0, H_0*W_0, ...]
            input_padding_mask: (N, \sum_{l=0}^{L-1} H_l \cdot W_l), True for padding
        ...
        if (input_spatial_shapes[:, 0] * input_spatial_shapes[:, 1]).sum() != Len_in:
            msg = "Spatial shapes product sum does not match input length"
            raise ValueError(msg)
        ...
        if reference_points.shape[-1] == 2:  # noqa: PLR2004
            ...
            raise ValueError(msg)
        attention_weights = f_nn.softmax(attention_weights, -1)
