...
from .tm_utils import build_1d_sincos_posemb, build_2d_sincos_posemb, interpolate_pos_encoding, pair
...
class SequenceDecoderEmbedding(nn.Module):
    ...
    def init(self, dim_tokens: int = 768, init_std=0.02):
        ...
        self.token_emb = nn.Embedding(
            num_embeddings=self.vocab_size, embedding_dim=self.dim_tokens, padding_idx=self.padding_idx
        )
    ...
    def forward_embed(self, d: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        target_pos_emb = torch.gather(
            expanded_pos_emb, dim=1, index=repeat(target_pos_id, "b n -> b n d", d=expanded_pos_emb.shape[2])
        )
    ...
    def forward_embed(self, d: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        if (input_size, input_size) != self.image_size:
            ...
            pos_emb = interpolate_pos_encoding(
                self.pos_emb.clone(), input_size, input_size, self.patch_size, self.dim_tokens
            )
        else:
